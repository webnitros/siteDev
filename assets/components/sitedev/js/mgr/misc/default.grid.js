siteDev.grid.Default = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        baseParams: {},
        cls: config['cls'] || 'main-wrapper sitedev-grid',
        autoHeight: true,
        paging: true,
        remoteSort: true,
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                    cls.push('sitedev-row-unpublished');
                }
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('sitedev-row-inactive');
                }
                if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                    cls.push('sitedev-row-deleted');
                }
                if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                    cls.push('sitedev-row-required');
                }
                return cls.join(' ');
            }
        },
    });
    siteDev.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(siteDev.grid.Default, MODx.grid.Grid, {
...
Ext.reg('sitedev-grid-default', siteDev.grid.Default);